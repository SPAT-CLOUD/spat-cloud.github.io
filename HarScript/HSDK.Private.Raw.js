...
class HarScript extends LowCrypt
{
    ...
    rh=()=>{
      ...
      sp.upper=function(){return this.toLocaleUpperCase();};
      sp.lower=function(){return this.toLowerCase();};       
      ...
      sp.rCrypt=function(){
            ...
            var  crz=this.split('');
             var mac=crz.map(npt=>String.fromCharCode((Regt-3)-npt.charCodeAt(0)));
             return mac.toString().split(',').join("");
       };
    ...
    sp.binStr=function(bsz=8){
       var sbs=this.split('').map(mst=>mst.charCodeAt(0).toString(2).padStart(bsz,"0"));
        var nog=sbs
...
var mxd=[];
              var cpp=0;
for(cpp;cpp<tss.length/8;cpp++){
   mxd.push(tss.slice(cpp*8,(cpp+1)*8));
}
